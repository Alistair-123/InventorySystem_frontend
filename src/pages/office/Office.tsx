...
function Offices() {
   const [isOpen, setIsOpen] = useState(false);
    ...
    const [office, setOffice] = useState<GetOffice[]>([]);
    const [page, setPage] = useState(1);
    const [totalPages, setTotalPages] = useState(1);
    const limit = 20;
    const [search, setSearch] = useState("");
    const [mode, setMode] = useState<"create" | "edit">("create");
    const [pendingEditData, setPendingEditData] = useState<CreateOffice | null>(
      null
    );
    const [isProcessing, setIsProcessing] = useState(false);
    const [confirmOpen, setConfirmOpen] = useState(false);
    const [confirmType, setConfirmType] = useState<"delete" | "edit">("delete");
    const [selectedOffice, setSelectedOffice] = useState<GetOffice | null>(null);
     ...
     const fetchData = useCallback(async () => {
        try {
          ...
          const res = await axiosInstance.get("/office/getoffices", {
            params: {
              page,
              limit,
              search,
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
    ...
    toastError("Failed to create office");
      ...
      const handleDeleteClick = (office: GetOffice) => {
        setSelectedOffice(office);
  ...
  return (
    <div className='font-poppins'>
      ...
      <Table className='p-4 '>
        ...
        <TableHeader>
         ...
         <TableBody>
            ...
            {!isLoading && !isError && office.length === 0 && (
              <TableRow>
              ...
              office.map((office, index) => {
                ...
                return (
                  <TableRow key={office._id}>
                    ...
                    <TableCell>{office.officeId}</TableCell>
      ...
      <Modal
        ...
        title={mode === "edit" ? "Edit Office" : "Create Office"}
              ...
              description={
                confirmType === "delete" ? (
                  ...
                  <>
                    ...
                    <span className="font-semibold">{selectedOffice?.officeName}</span>?
                  </>
                )
              }
